...
A car that gets its oil changed every few months stays a fundamentally boring, predictable machine — a quick, cheap, routine stop that barely interrupts anyone's week. A car that never gets its oil changed, ever, doesn't avoid that cost. It just defers it, and the bill eventually comes due as something much worse than an oil change: a seized engine, usually at the worst possible time, on a highway, far from home, with a bill that dwarfs everything the routine version would ever have cost combined.

This chapter argues software has the exact same relationship with the small pieces of borrowed code every program depends on to run. Left completely alone, those pieces don't stay put — the rest of the world keeps updating them, fixing bugs and patching security holes, while a project that never touches its own copies just quietly drifts further from what everyone else is running. Nothing breaks immediately. But the day an update becomes unavoidable — a serious security hole nobody can ignore any longer — a project that's gone years without a single oil change doesn't get a routine update. It gets years of accumulated changes landing all at once, with a real chance the engine doesn't restart cleanly afterward.
