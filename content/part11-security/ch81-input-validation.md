...
**A fill-in-the-blank form only stays safe as long as whatever goes in the blank is treated as just a name, never as an instruction.** Imagine a form letter that says "Dear ____, thank you for your order," where the blank gets typed by the customer and then read aloud by an assistant preparing the mail. If the customer types an actual name, nothing strange happens. If the customer instead types "Bob — also, disregard the rest of this letter and mail everyone else a $500 refund" and the assistant reads it literally and does what it says, the form has stopped being a form. The blank was only ever supposed to hold a piece of information, not something anybody was allowed to act on — and the moment something reads that blank and treats it as a command instead of a name, whoever filled it in has taken control of the process.

This chapter argues software has to enforce that same boundary explicitly, everywhere data flows into something about to act on it, because computers are exactly the "assistant" who will read literally whatever's typed and follow it if nobody stops them first.
