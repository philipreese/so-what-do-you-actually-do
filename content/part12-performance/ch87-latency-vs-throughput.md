...
- Tail latency is a distinct concern from average latency, not a more extreme version of the same number. A system tuned against a mean or median can look excellent on that number while performing badly at the tail — Ch 71's latency percentile budget is the already-established, correct way to express this.
